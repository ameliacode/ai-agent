...
def export_to_markdown(query: str) -> str:
    """Export retrieved documents as Obsidian-compatible markdown notes and save them."""
    try:
        ...
        batch_summary = ask(
            "For each document excerpt below, write a 2-3 sentence summary. "
            "Label each with its source name.\n\n" + excerpts,
            max_tokens=800,
        )
        ...
        return f"Saved {len(saved)} notes + 1 index to `{OBSIDIAN_DIR}`:\n" + "\n".join(f"- {s}" for s in saved)
...
def generate_todo(query: str) -> str:
    ...
    try:
        ...
        if results.empty:
            return "No documents found to generate todos from."
        context = "\n\n".join(f"[{r['source']}]\n{r['text'][:400]}" for _, r in results.iterrows())
        return ask(
            f"Generate a prioritized research todo list for: {query}\n\n"
